defmodule PhoenixKitDocumentCreator.LiveDatabaseGuardWiringTest do
  @moduledoc """
  S014: `PhoenixKitDocumentCreator.Test.LiveDatabaseGuardTest` calls
  `check!/1` directly — it proves the logic is correct, not that
  `test_helper.exs` actually calls it. Deleting the call from
  `test_helper.exs` leaves that test green; it would prove the module
  still works, not that anything still protects a real `mix test` run.
  ...
  test "an isolated test database name is not refused — the guard does not block a real run" do
    ...
    env = [{"PGDATABASE", System.get_env("PGDATABASE", "phoenix_kit_document_creator_test")}]
